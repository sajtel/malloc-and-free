...
void *mm_malloc(size_t size)
{
  ...
  //case that call requires more heap space - call requestMoreSpace
  if((ptrAllBlock = searchFreeList(reqSize)) == NULL){
    requestMoreSpace(reqSize);
    ptrAllBlock = searchFreeList(reqSize);
  }

  blockSize = SIZE(ptrAllBlock->sizeAndTags);   //size of freeblock
  precedingBlockUseTag = ptrAllBlock->sizeAndTags & TAG_PRECEDING_USED;


  if((blockSize - reqSize) >= MIN_BLOCK_SIZE){
    //1. find free block size
    size_t freeBlockSize = blockSize - reqSize;
    //2. get ptr to free block
    ptrFreeBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrAllBlock,reqSize);
    //3. set the preceding used tag of the new free block in the header
    ptrFreeBlock->sizeAndTags = (freeBlockSize | TAG_PRECEDING_USED);
    ptrFreeBlock->sizeAndTags = ptrFreeBlock->sizeAndTags & (~0<<1);  //set use bit to 0
    //4. set the preceding used tag of the new free block in the footer
    *((size_t*)(UNSCALED_POINTER_ADD(ptrFreeBlock,freeBlockSize-WORD_SIZE))) = ptrFreeBlock->sizeAndTags;
    //insert the block into the freelist and coalesce 
    insertFreeBlock(ptrFreeBlock);
    coalesceFreeBlock(ptrFreeBlock);
    //set the header of the allocated block to indicate new size
    //set the header of the allocated block 
    //reqsize | precedingBlockUseTag | TAG_USED
    ptrAllBlock->sizeAndTags = (reqSize| TAG_USED);  //make size of allocated block to reqsize and set used bit
    ptrAllBlock->sizeAndTags |= precedingBlockUseTag;  //set tag of allocated block to preceding block tag before allocating

  }else{
    //blockSize < MIN_BLOCK_SIZE
    //set header of allocated block to indicate new size
    //set preceding tag used of next block in the footer
    //set preceding tag used of next block in the header
    ptrAllBlock->sizeAndTags = ptrAllBlock->sizeAndTags | TAG_USED;
    BlockInfo* successorHeader = (BlockInfo*)UNSCALED_POINTER_ADD(ptrAllBlock,blockSize);
    successorHeader->sizeAndTags|= TAG_PRECEDING_USED;
